...
dC = get_criticalValuesFLLM(dAlpha) # Critical values
mRej = matrix(0.0, ncol = length(vT), nrow = length(vN)) # Pre-allocation matrix for rejection frequencies

for(n in seq_along(vN)){
  iN = vN[n]
  
  # Simulate data. Use Type = "size" for under the null and Type = "power" for the alternative
  lInput = list(iK = iK, iN = iN, iT = iT.max, Phi = mPhi, Type="power",
